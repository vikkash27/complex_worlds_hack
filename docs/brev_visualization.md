...
For the more impressive humanoid showcase, run the same replay script with
`--humanoid-showcase` and the `artifacts/traces/humanoid_*_long_horizon.jsonl`
traces generated by `scripts/run_demo.py`.
